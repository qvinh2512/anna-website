...
export default function LightboxGallery({ images }: { images: string[] }) {
  ...
  useEffect(() => {
    const results: boolean[] = new Array(images.length).fill(false)
    let loaded = 0
  ...
  const getItemClass = (i: number) => {
    ...
    return orientations[i] === false ? 'col-span-2' : ''
  }

  const gridClass = images.length === 1 ? 'flex justify-center' : 'grid grid-cols-2 gap-3'
      ...
      <div className={gridClass}>
        {images.map((img, i) => (
          <div key={i} onClick={() => setIndex(i)}
            className={`rounded-xl overflow-hidden shadow-md cursor-pointer hover:shadow-xl hover:scale-[1.02] transition-all relative group ${getItemClass(i)}`}>
