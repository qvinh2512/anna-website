...
export default function LightboxGallery({ images }: { images: string[] }) {
  ...
  const imgCard = (src: string, i: number, extraClass = '', style?: React.CSSProperties) => (
    <div key={i} onClick={() => setIndex(i)}
      className={`rounded-xl overflow-hidden shadow-md cursor-pointer active:scale-95 hover:shadow-xl hover:scale-[1.02] transition-all relative group ${extraClass}`}
      style={style}>
  ...
  const renderGrid = () => {
    if (images.length === 1) return (
      <div className="flex justify-center">
        <div onClick={() => setIndex(0)} className="rounded-xl overflow-hidden shadow-md cursor-pointer active:scale-95 hover:shadow-xl transition-all relative group max-w-lg w-full">
    ...
    <>
      {index !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90"
          onClick={close} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          <button onClick={close} className="absolute top-4 right-4 text-white/70 hover:text-white w-12 h-12 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-2xl z-10">✕</button>
          <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white/60 text-sm bg-black/30 px-3 py-1 rounded-full">{index + 1} / {images.length}</div>
          {images.length > 1 && (
            <button onClick={e => { e.stopPropagation(); prev() }} className="absolute left-2 md:left-4 text-white w-14 h-14 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 active:bg-white/30 text-4xl z-10">‹</button>
          )}
          ...
          {images.length > 1 && (
            <button onClick={e => { e.stopPropagation(); next() }} className="absolute right-2 md:right-4 text-white w-14 h-14 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 active:bg-white/30 text-4xl z-10">›</button>
          )}
          {images.length > 1 && (
          ...
          {images.length > 1 && (
            <div className="absolute bottom-12 left-1/2 -translate-x-1/2 text-white/40 text-xs md:hidden">Vuốt để chuyển ảnh</div>
          )}
        </div>
      )}
      {renderGrid()}
